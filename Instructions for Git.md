...
```
Переходы между ветками осуществляются аналогично переходам между комитами, с той лишь разницей, что вместо первых символов комита прописывается имя ветки в которую мы хотим попасть.

* Слияние веток
```
git merge имя_ветки
```
Данная команда проивзодит перенос данных из одной ветки в другу.
